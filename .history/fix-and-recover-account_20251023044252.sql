-- Script para adicionar coluna currency e recuperar conta principal
-- Execute este script no Supabase SQL Editor

-- 1. Primeiro, adicionar a coluna currency se não existir
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ADD COLUMN currency TEXT DEFAULT 'kr';
        
        RAISE NOTICE 'Coluna currency adicionada à tabela accounts';
    ELSE
        RAISE NOTICE 'Coluna currency já existe na tabela accounts';
    END IF;
END $$;

-- 2. Agora recuperar a conta principal
WITH user_data AS (
...
GROUP BY a.id, a.name, a.type, a.currency, u.email;
