...
/**
 * Format currency value with specific currency
 *
 * @example
 * formatCurrencyWithSymbol(1234.56, "kr") // "1 234,56 kr"
 * formatCurrencyWithSymbol(1234.56, "real") // "R$ 1.234,56"
 */
export function formatCurrencyWithSymbol(value: number, currency: string): string {
  const currencyMap = {
    kr: { locale: "sv-SE", currency: "SEK", symbol: "kr" },
    real: { locale: "pt-BR", currency: "BRL", symbol: "R$" },
    dolar: { locale: "en-US", currency: "USD", symbol: "$" },
    euro: { locale: "de-DE", currency: "EUR", symbol: "€" },
  };

  const config = currencyMap[currency as keyof typeof currencyMap] || currencyMap.kr;
