    ...
    try {
      ...
      if (localSettings) {
        ...
        setSettings({
          ...
          auto_adjust: parsed.auto_adjust !== undefined ? parsed.auto_adjust : true,
          notifications_enabled: parsed.notifications_enabled !== undefined ? parsed.notifications_enabled : true,
        });
        setIsLoading(false);
        return;
      }
      ...
      if (error && error.code !== "PGRST116") {
        ...
        console.log("Tabela account_forecast_settings não existe ainda, usando configurações padrão");
        setSettings({
          monthly_budget: null,
          alert_threshold: 80,
  ...
  const updateSettings = async (newSettings: Partial<ForecastSettings>) => {
    try {
      ...
      try {
        ...
        if (error) {
          console.log("Tabela account_forecast_settings não existe ainda, salvando apenas no localStorage");
        }
      } catch (dbError) {
