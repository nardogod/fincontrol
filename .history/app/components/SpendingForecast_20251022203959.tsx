...
export default function SpendingForecast({
  ...
  const forecastData = useMemo((): ForecastData => {
    ...
    if (customSettings) {
      console.log("Custom settings encontradas:", customSettings);
      if (
        customSettings.budget_type === "fixed" &&
        customSettings.monthly_budget
      ) {
        monthlyEstimate = customSettings.monthly_budget;
        console.log("Usando orçamento fixo:", monthlyEstimate);
      } else if (
      ...
      } else if (customSettings.monthly_budget) {
        // Usar orçamento personalizado como fallback
        monthlyEstimate = customSettings.monthly_budget;
        console.log("Usando orçamento personalizado:", monthlyEstimate);
      }
    } else {
      console.log("Nenhuma configuração personalizada encontrada");
    }
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 bg-white rounded-lg border">
            ...
            <p className="text-xs text-gray-500">Baseado nos últimos 6 meses</p>
          </div>
