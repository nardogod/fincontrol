...
export default function SpendingForecast({
  ...
  const forecastData = useMemo((): ForecastData => {
    ...
    // Calcular valor restante baseado na estimativa
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between p-4 bg-white rounded-lg border">
          ...
          <div className="text-right">
            ...
            <p className="text-sm font-medium">{getConfidenceLabel(forecastData.confidence)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 bg-white rounded-lg border">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-gray-700">Gasto Estimado/Mês</span>
            </div>
          ...
          <div className="p-4 bg-white rounded-lg border">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-gray-700">Restante Este Mês</span>
            </div>
            <p className={`text-2xl font-bold ${forecastData.remainingThisMonth > 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span>{Math.round((forecastData.currentWeekSpent / forecastData.monthlyEstimate) * 100)}%</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div 
              ...
              style={{ 
                width: `${Math.min(100, (forecastData.currentWeekSpent / forecastData.monthlyEstimate) * 100)}%` 
              }}
            />
          </div>
          <div className="flex justify-between text-xs text-gray-500">
            <span>Gasto atual: {formatCurrency(forecastData.currentWeekSpent)}</span>
            <span>Meta mensal: {formatCurrency(forecastData.monthlyEstimate)}</span>
          </div>
        </div>
        ...
        {forecastData.status === 'over-budget' && (
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
            ...
            <p className="text-sm text-red-700 mt-1">
              Você está gastando acima da sua média histórica. Considere revisar seus gastos.
            </p>
          </div>
        )}
        ...
        {forecastData.status === 'under-budget' && (
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-blue-800">Bom trabalho!</span>
            </div>
