...
export default function AccountsForecastSummary({
  ...
  const accountsForecast = useMemo((): AccountForecast[] => {
    ...
    return accounts.map((account) => {
      ...
      return {
        ...
        status,
        progress:
          monthlyEstimate > 0 ? (currentMonthSpent / monthlyEstimate) * 100 : 0,
        daysRemaining,
      };
    });
  ...
  const totalMonthlyEstimate = accountsForecast.reduce(
    (sum, f) => sum + f.monthlyEstimate,
    0
  );
  const totalCurrentSpent = accountsForecast.reduce(
    (sum, f) => sum + f.currentMonthSpent,
    0
  );
  const totalWeekSpent = accountsForecast.reduce(
    (sum, f) => sum + f.currentWeekSpent,
    0
  );
            ...
            <div className="flex justify-between text-sm mb-2">
              ...
              <span>
                {totalMonthlyEstimate > 0
                  ? Math.round((totalCurrentSpent / totalMonthlyEstimate) * 100)
                  : 0}
                %
              </span>
            </div>
              ...
              <div
                ...
                style={{
                  width: `${Math.min(
                    100,
                    totalMonthlyEstimate > 0
                      ? (totalCurrentSpent / totalMonthlyEstimate) * 100
                      : 0
                  )}%`,
                }}
              />
              ...
              {forecast.status === "over-budget" && (
                <div className="mt-3 p-2 bg-red-100 border border-red-200 rounded text-xs">
                  ...
                  <p className="text-red-700 mt-1">
                    Acima da média em{" "}
                    {formatCurrency(
                      forecast.currentMonthSpent - forecast.monthlyEstimate
                    )}
                  </p>
                </div>
              )}
